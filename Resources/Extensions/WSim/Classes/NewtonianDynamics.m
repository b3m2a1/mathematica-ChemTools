...
NewtDynPropC[force_, field_, friction_, rCut_]:=
	With[
		{
			GE=Compile`GetElement
			},
					...
					moduleHold[
						...
						Do[
							...
							Do[
								...
								Do[
									If[i!=j,
										dispVec=wCoord-GE[coordVec, j];
										disp=Sqrt@Total[dispVec^2];
										If[disp>0.&&disp<rCut,
											forceVal+=dispVec*force[disp, totT]/disp
											];
										],
									{j, walkerNum}
									];
								(* Update walker i position from velocity Verlet *)
								wCoord=wCoord+dt*velOld+dt^2*forceVal/2;
								coordVec[[i]]=wCoord;
								If[n>1,
									velVec[[i]]=(wCoord-coordVecOld[[i]])/(2*dt),
									velVec[[i]]=velOld+dt*forceVal
									],
								{i, walkerNum}
								];
							(* Compute new velocity of walker i *)
							totT+=dt;
...
NewtDynPropFunc//Clear


NewtDynPropFunc[obj_]:=
	Module[
		{
			force=obj["InteractionFunction"],
			field=obj["FieldForceFunction"],
			fric=obj["FrictionFunction"],
			rCut=obj["InteractionRadius"]
			},
		NewtDynPropC[
			force,
			field,
...
NewtDynWalkerBoxFunction[walks_]:=
...
NewtDynPlotState//Clear


NewtDynPlotState[walkers_, cbs_, t_, obj_, ops:OptionsPattern[]]:=
...
NewtDynPlotVelocities//Clear


Options[NewtDynPlotVelocities]=
	Options[ListVectorPlot3D];
NewtDynPlotVelocities[{coords_, vels_}, cbs_, t_, obj_, ops:OptionsPattern[]]:=
