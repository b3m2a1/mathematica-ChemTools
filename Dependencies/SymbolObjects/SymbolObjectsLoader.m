...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated SymbolObjects loader file *)


Unprotect[SymbolObjects`PackageScope`Private`$TopLevelLoad];
SymbolObjects`PackageScope`Private`$TopLevelLoad=
...
SymbolObjects::usage=
	"SymbolObjects is a manager head for the SymbolObjects package";
...
SymbolObjects["PackageListing"]:=$PackageListing;
$PackageListing=<||>;
SymbolObjects["Contexts"]:=$PackageContexts;
$PackageContexts={
		"SymbolObjects`",
		"SymbolObjects`PackageScope`Private`",
		"SymbolObjects`PackageScope`Package`"
		};
$PackageDeclared=
...
SymbolObjects["FEScopedSymbols"]:=$PackageFEHiddenSymbols;
$PackageFEHiddenSymbols={};
SymbolObjects["PackageScopedSymbols"]:=$PackageScopedSymbols;
$PackageScopedSymbols={};
SymbolObjects["LoadingParameters"]:=$PackageLoadSpecs
$PackageLoadSpecs=
	Merge[
		{
			With[
...
SymbolObjects["AllowRescoping"]:=$AllowPackageRescoping;
$AllowPackageRescoping=
	Replace[
		Lookup[$PackageLoadSpecs, "AllowRescoping"],
		Except[True|False]->$TopLevelLoad
		];
SymbolObjects["AllowRecoloring"]:=$AllowPackageRecoloring;
$AllowPackageRecoloring=
	Replace[
		Lookup[$PackageLoadSpecs, "AllowRecoloring"],
		Except[True|False]->$TopLevelLoad
		];
SymbolObjects["AllowAutocompletions"]:=$AllowPackageAutocompletions;
...
SymbolObjects["FileContexts"]:=$PackageFileContexts;
...
SymbolObjects["DeclaredPackages"]:=$DeclaredPackages;
...
SymbolObjects["LoadedPackages"]:=$LoadedPackages;
...
PackageFileContext[f_String?DirectoryQ]:=
	With[{s=PackageFileContextPath[f]},
		StringRiffle[Append[""]@Prepend[s,SymbolObjects],"`"]
		];
...
SymbolObjects::nodep="Couldn't load dependency `` of type ``";
SymbolObjects::nodup="Couldn't update dependency `` of type ``";
	...
	Block[{retcode, site, path, file, tmp},
		...
		site=
			...
			If[retcode<300,
				...
				DeleteFile[tmp];
				file,
				Message[SymbolObjects::nodep, dep, "Package"];
				DeleteFile[tmp];
				$Failed
				]
			];
		...
		Monitor[
			MapThread[
				Check[
					...
					Message[SymbolObjects::nodep, #2, "Paclet"];
					$Failed
					]&,
				{
					pacs,
			...
			Monitor[
				MapThread[
					Check[
						...
						Message[SymbolObjects::nodup, #2, "Paclet"];
						$Failed
						]&,
					{
						StringDelete[deps, "`"],
...
$PackageErrorMessage=
	"SymbolObjects encountered exception ``";
...
PackageCheck[
	...
	Replace[
		Thread[Map[Hold[MessageName[SymbolObjects, #]]&, msg], Hold],
		{
			{}:>
				Check[expr, 
...
PackageFailure[
	...
	(
		Set[MessageName[SymbolObjects, tag], body];
		PackageFailure[
			MessageName[SymbolObjects, tag],
			body,
			ops
...
PackageAttachMainAutocomplete[]:=
	PackageAddAutocompletions[SymbolObjects, 
		Table[
			Replace[{}->None]@
				Cases[
					DownValues[SymbolObjects],
					Nest[
						Insert[#, _, {1, 1, 1}]&,
						(HoldPattern[Verbatim[HoldPattern]][
							SymbolObjects[s_String, ___]
							]:>_),
						n-1
						]:>s,
					Infinity
...
If[
	(Clear@SymbolObjects`PackageScope`Private`$loadAbort;!#)&@
		SymbolObjects`PackageScope`Private`$loadAbort,
	Unprotect[SymbolObjects`PackageScope`Private`$LoadCompleted];
	SymbolObjects`PackageScope`Private`$LoadCompleted=True;
	SymbolObjects`PackageScope`Private`PackagePostProcessContextPathReassign[]
	]