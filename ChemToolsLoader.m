...
Unprotect[ChemTools`Private`Package`$TopLevelLoad];
ChemTools`Private`Package`$TopLevelLoad=
...
$PackageContexts={
		"ChemTools`",
		"ChemTools`Private`Package`",
		"ChemTools`Private`Hidden`"
		};
$PackageDeclared=
...
PackageScopeBlock[e_,scope_String:"Hidden"]/;TrueQ[$AllowPackageRescoping]:=
	With[{newcont="ChemTools`Private`"<>StringTrim[scope,"`"]<>"`"},
...
PackageDecontext[
	...
	With[{
		...
		ctx="ChemTools`Private`"<>StringTrim[scope,"`"]<>"`"
		},
		Replace[names,
...
If[(Clear@ChemTools`Private`Package`$loadAbort;!#)&@ChemTools`Private`Package`$loadAbort,
	ChemTools`Private`Package`PackagePostProcessContextPathReassign[]
	]